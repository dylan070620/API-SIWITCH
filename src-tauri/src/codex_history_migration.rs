...
pub(crate) fn collect_jsonl_files(dir: &Path, files: &mut Vec<PathBuf>, depth: u8, max_depth: u8) {
...
pub(crate) fn ensure_codex_session_file_unchanged(
    path: &Path,
...
pub(crate) fn codex_state_db_paths(codex_dir: &Path, config_text: &str) -> Vec<PathBuf> {
...
pub(crate) fn backup_codex_jsonl_file(
    path: &Path,
    codex_dir: &Path,
    backup_root: &Path,
) -> Result<(), AppError> {
...
pub(crate) fn backup_codex_state_db(
    db_path: &Path,
    codex_dir: &Path,
    backup_root: &Path,
