...
/// 让切换 provider 后"消失"的历史会话重新可见。仅操作本机 `~/.codex` 数据。
#[tauri::command]
pub async fn sync_codex_sessions(
) -> Result<crate::codex_session_sync::CodexSessionSyncOutcome, String> {
    tauri::async_runtime::spawn_blocking(
        crate::codex_session_sync::sync_codex_sessions_to_current_provider,
    )
    .await
    .map_err(|e| format!("Failed to run Codex session sync: {e}"))?
