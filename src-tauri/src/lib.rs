...
pub fn run() {
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 API Switch 的工作方式。
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.apiswitch.desktop/applications/cc-switch-handler.desktop
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older API Switch version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            2) If you see “database version is newer”, please upgrade API Switch\n\
