...
function App() {
  ...
  // Codex 会话同步（仅 Codex 应用展示同步按钮时使用）
  const { sync: syncCodexSessions, isSyncing: isCodexSyncing } = useCodexSync();
                  ...
                  <>
                    {activeApp === "codex" && (
                      <Button
                        ...
                        onClick={() => syncCodexSessions()}
                        title={t("codexSync.button", {
                          defaultValue: "同步会话",
                        })}
                        className="shrink-0 text-muted-foreground hover:text-foreground hover:bg-black/5 dark:hover:bg-white/5"
                      >
                        <RefreshCw
                          className={cn(
                            "w-4 h-4",
                            isCodexSyncing && "animate-spin",
                            !isToolbarCompact && "mr-2",
                          )}
                        />
                        {!isToolbarCompact &&
                          (isCodexSyncing
