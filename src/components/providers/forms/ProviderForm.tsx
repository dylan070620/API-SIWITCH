...
import { isPresetVisible } from "@/config/visiblePresets";
...
function ProviderFormFull({
  ...
  const presetEntries = useMemo(() => {
    // 注意：先 map 再 filter，保证可见项的 id 仍等于其在原始数组中的下标，
    // 这样 AddProviderDialog 按裸下标反查 endpointCandidates 不会错位。
    if (appId === "codex") {
      return codexProviderPresets
        ...
        .filter((e) => isPresetVisible("codex", e.preset.name));
      ...
      return geminiProviderPresets
        ...
        .filter((e) => isPresetVisible("gemini", e.preset.name));
      ...
      return opencodeProviderPresets
        ...
        .filter((e) => isPresetVisible("opencode", e.preset.name));
      ...
      return openclawProviderPresets
        ...
        .filter((e) => isPresetVisible("openclaw", e.preset.name));
      ...
      return hermesProviderPresets
        ...
        .filter((e) => isPresetVisible("hermes", e.preset.name));
    }
    return providerPresets
      ...
      .filter(
        (e) =>
          !(e.preset as ProviderPreset).hidden &&
          isPresetVisible("claude", e.preset.name),
      );
  }, [appId]);
