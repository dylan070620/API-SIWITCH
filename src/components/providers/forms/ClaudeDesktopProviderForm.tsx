  ...
  const presetEntries = useMemo<PresetEntry[]>(
    () =>
      claudeDesktopProviderPresets
        ...
        .filter((e) => isPresetVisible("claude-desktop", e.preset.name)),
    [],
  );
                  ...
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    {needsModelMapping
                      ? t("claudeDesktop.modelMappingOnHint", {
                          defaultValue:
                            "Claude Desktop 只接受 claude-sonnet-* / claude-opus-* / claude-haiku-* 三档角色 ID。开启后 API Switch 会把这三档映射到供应商的实际模型，并在使用期间保持本地路由开启。",
                        })
