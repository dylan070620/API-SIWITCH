...
import { isOfficialIcon } from "@/config/officialIcons";
...
export const IconPicker: React.FC<IconPickerProps> = ({
  ...
  // 过滤图标列表：只显示官方图标（中转站/非官方图标的 SVG 仍保留，仅从选择器隐藏）
  const filteredIcons = useMemo(() => {
    if (!searchQuery) return iconList.filter(isOfficialIcon);
    return searchIcons(searchQuery).filter(isOfficialIcon);
  }, [searchQuery]);
